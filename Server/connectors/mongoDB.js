...
//const Tiposdocu = require('../models/mongoDB/tiposdocu');
...
const addTiposdocu_C = input => {
  input.roles = ["dummy"]; // assign a dummy roles at first time user is created
  let tipo = new Tiposdocu(input);
  return Tiposdocu.find({ codigo: input.codigo }).then((res) => {
    if (res.length > 0) {
      return { descripcion: "", codigo: "", message : "Codigo Existente" };
    } else {
      tipo.save();
      return input
    }
...
// Tipos  codes functions
const setTiposdocuCode_C = input => {
  let tiposdocucode = new TiposdocuCode(input);
  return TiposdocuCode.find({ code: input.code }).then((res) => {
    if (res != "") {
...
const setTiposdocuCodeDoc_C = input => {
  let tiposdocucode = new TiposdocuCode(input);
  return TiposdocuCode.findByIdAndUpdate(input._id, input, function (err, res) {
    if (err) {
      console.log(err);
    }
    if (res) {
...
const delTiposdocuCodeDoc_C = input => {
return TiposdocuCode.findByIdAndRemove(input._id, function (err, res) {
  if (err) {
    console.log(err);
  }
  if (res) {
...
const getTiposdocuCode_C = input => {
  return checkRoles(input).then((res) => {
    if (res.authenticated) {
      if (input._id) {
        // search OneDocByID
        var ObjectId = require('mongoose').Types.ObjectId;
        return TiposdocuCode.findOne({ _id: new ObjectId(input._id) }).then((res)=> {
          return [res];
        });
      } else {
        // search all docs
        return TiposdocuCode.find({ code: { "$regex": input.code, "$options": "i" }, descr: { "$regex": input.descr, "$options": "i" } }).then((res) => {
          if (res != "") {
            return res;
          } else {
            return [{ _id: "", code: "", descr: "", message: "Success" }];
